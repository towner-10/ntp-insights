...
export function ScanContextMenu(props: {
	scan: Scan;
	link: string;
	onRefresh: () => void;
}) {
	const toaster = useToast();
	const archivePath = api.scans.archive.useMutation();
	const renamePath = api.scans.rename.useMutation();
	const {
		register,
		handleSubmit,
		setValue,
	...
	const onSubmit = (event: FormEvent<HTMLFormElement>) => {
		...
		void (async () => {
			await handleSubmit((async (data) => {
				const scan = await renamePath.mutateAsync({
					id: props.scan.id,
					name: data.name,
				});
				...
				if (scan) {
					toaster.toast({
						title: 'Path renamed.',
						description: 'The scan has been renamed.',
						duration: 5000,
					});
		...
		<Dialog open={showRenameDialog} onOpenChange={setShowRenameDialog}>
			...
			>
				<DropdownMenu>
					...
					<DropdownMenuContent align="end">
						...
						<DropdownMenuSeparator />
						<Link href={`/lidar/${props.scan.id}/view`}>
				...
				<AlertDialogContent>
					...
					<AlertDialogFooter>
						...
						<AlertDialogAction
							onClick={() => {
								void archivePath.mutateAsync({ id: props.scan.id }).then(() => {
									toaster.toast({
										title: 'Path archived.',
			...
			<DialogContent>
				<DialogHeader>
					...
					<DialogDescription>
						<p>Enter a new name for this scan.</p>
					</DialogDescription>
				</DialogHeader>
